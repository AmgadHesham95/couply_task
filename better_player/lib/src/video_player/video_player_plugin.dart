...
class VideoPlayerPlugin extends VideoPlayerPlatform {
  ...
  @override
  Future<int?> create(
      {BetterPlayerBufferingConfiguration? bufferingConfiguration, BetterPlayerDataSource? dataSource}) async {
    ...
    if (dataSource != null) {
      ...
      _videoPlayers[textureId] = _VideoPlayer(uri: url, textureId: textureId, headers: headers);
  ...
  Future<DateTime?> getAbsolutePosition(int? textureId) async {
    final int milliseconds = _videoPlayers[textureId]!.getPosition().inMilliseconds;
    return DateTime.fromMillisecondsSinceEpoch(milliseconds);
  }

  @override
  Future<void> setTrackParameters(int? textureId, int? width, int? height, int? bitrate, int? trackId) async {
    if (trackId != null) {
  ...
  @override
  Future<void> setMixWithOthers(int? textureId, bool mixWithOthers) => Future<void>.value();
...
class _VideoPlayer {
  _VideoPlayer({required this.uri, required this.textureId, required this.headers});
  ...
  void setBuffering(bool buffering) {
    if (isBuffering != buffering) {
      ...
      eventController.add(
          VideoEvent(key: '', eventType: isBuffering ? VideoEventType.bufferingStart : VideoEventType.bufferingEnd));
    }
  }
  ...
  void initialize() {
    ...
    videoElement.onEnded.listen((dynamic _) {
      setBuffering(false);
      eventController.add(VideoEvent(key: '', eventType: VideoEventType.completed));
    });
  }
  ...
  void seekTo(Duration position) => videoElement.currentTime = position.inMilliseconds.toDouble() / 1000;
